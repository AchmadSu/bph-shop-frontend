...
export default function BuyerDashboard() {
  ...
  const addToCart = async (product) => {
    try {
      ...
      setToast({ show: true, message: `${product.name} added to cart!`, variant: "success" });
      fetchCart();
    } catch (err) {
      const message = err.response?.data.message;
      setToast({ show: true, message: message ?? `Failed to add ${product.name}`, variant: "danger" });
    }
  };
  ...
  const removeFromCart = async (cartItemId) => {
    try {
      ...
      const message = err.response?.data.message;
      setToast({ show: true, message: message ?? `Failed to remove item`, variant: "danger" });
    }
  };
  ...
  const updateQuantity = async (cartItemId, quantity) => {
    try {
      ...
      const message = err.response?.data.message;
      setToast({ show: true, message: message ?? `Failed to update quantity`, variant: "danger" });
    }
  };
  ...
  const checkout = async () => {
    ...
    try {
      ...
      setToast({ show: true, message: "Checkout successful!", variant: "success" });
  ...
  return (
    <Container className="mt-4">
          ...
          <Col key={product.id} md={4} lg={3} className="mb-3">
            <Card className="h-100 d-flex flex-column">
              ...
              <Card.Body>
                ...
                <Button color="blue" onClick={() => addToCart(product)}><i class="cart arrow down icon"></i> Add to Cart</Button>
              </Card.Body>
            </Card>
          </Col>
        ))}
      ...
      <Toast
        ...
        autohide
        bg={toast.variant}
        style={{ position: "fixed", bottom: 100, right: 20, minWidth: 250 }}
      >
